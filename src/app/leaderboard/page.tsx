...
import {UserInfo} from "@/app/dashboard";

export default async function LeaderboardPage() {
    const supabase = await createClient();

    const data = await supabase.from("users").select("*");
    const leaderboard: UserInfo[] = data.data?.sort((a, b) => b.aura - a.aura).slice(0, 10) ?? [];
            ...
            <section>
                ...
                <ol>
                    {leaderboardData.map(({user, rank}) => (
                        <li key={user.id}>
                            <span><span>{rank}.</span> {user.name}</span> <span style={
                            {color: user.aura < 0 ? '#DA4167' : 'black'}
                        }>{user.aura} aura</span>
                        </li>
                    ))}
                </ol>
