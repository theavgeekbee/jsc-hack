...
export async function GET(): Promise<NextResponse> {
  ...
  if (
    row.data.completed_challenges &&
    row.data.completed_challenges.includes(id)
  ) {
    return NextResponse.json(
      { message: "Daily challenge already is completed" },
      { status: 420 }
    );
  }
  ...
  return NextResponse.json(
    { message: "Marked daily challenge as complete" },
    { status: 200 }
  );
}
