...
export default function Home() {
  ...
  const [user_info, setUserInfo] = useState<undefined | { data: UserInfo }>(
    undefined
  );
  ...
  const [last_week_challenges, setLastWeekChallenges] = useState<
    undefined | Challenge[]
  >(undefined);
  const [leaderboard, setLeaderboard] = useState<undefined | UserInfo[]>(
    undefined
  );
  ...
  const leaderboardPosition =
    leaderboard.findIndex((user) => user.name === user_info.data.name) + 1;
    ...
    <main className={styles.dashboard}>
      <section>
        ...
        <section className={styles.missions}>
          ...
          <div
            className={styles.mission_monthly}
